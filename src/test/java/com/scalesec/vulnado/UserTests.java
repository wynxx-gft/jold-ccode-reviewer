...
class UserTest {
    ...
    private static final String TEST_SECRET = "testSecretKeyForJWTTestingAtLeast32Bytes";
    ...
    // Helper method to capture System.err
    ...
    void constructor_ShouldInitializeAllFields() {
        ...
        String expectedHashedPassword = "testHashedPassword";

        User user = new User(expectedId, expectedUsername, expectedHashedPassword);
        ...
        assertEquals(expectedHashedPassword, user.hashedPassword, "User hashedPassword should be initialized correctly");
    }
    ...
    void token_ShouldContainCorrectUsername() {
        // Test that the token contains the correct username as subject
        String token = testUser.token(TEST_SECRET);
    ...
    void token_WithDifferentSecrets_ShouldGenerateDifferentTokens() {
        // Test that the same user with different secrets generates different tokens
        String secret1 = "secretKeyNumber1AtLeast32BytesLong";
        String secret2 = "secretKeyNumber2AtLeast32BytesLong";
    ...
    @Test
    void token_ShouldBeVerifiableWithSameSecret() {
        // Test that generated token can be verified with the same secret
        String token = testUser.token(TEST_SECRET);
        SecretKey key = Keys.hmacShaKeyFor(TEST_SECRET.getBytes());
    ...
    void assertAuth_WithWrongSecret_ShouldThrowUnauthorized() {
        ...
        String token = testUser.token(TEST_SECRET);
        String wrongSecret = "wrongSecretKeyAtLeast32BytesLongForTest";
    ...
    void fetch_ShouldExecuteQueryWithSQLInjectionVulnerability() throws Exception {
        // Test that fetch executes the SQL query (note: this demonstrates the SQL injection vulnerability)
        ...
        try (MockedStatic<Postgres> mockedPostgres = mockStatic(Postgres.class)) {
            ...
            // The vulnerable query includes SQL injection payload
    ...
    @Test
    void fetch_ShouldPrintDatabaseOpenedMessage() throws Exception {
    ...
    @Test
    void fetch_WithSQLInjectionAttempt_ShouldExecuteVulnerableQuery() throws Exception {
        // Test that demonstrates SQL injection vulnerability in fetch method
        ...
        try (MockedStatic<Postgres> mockedPostgres = mockStatic(Postgres.class)) {
            ...
            // Verify the vulnerable query is executed with the injection payload
            String expectedQuery = "select * from users where username = '" + maliciousUsername + "' limit 1DROP DATABASE 1";
            verify(mockStatement).executeQuery(expectedQuery);
        }
    }

    @Test
    void fetch_ShouldCorrectlyMapResultSetToUserObject() throws Exception {
        ...
        try (MockedStatic<Postgres> mockedPostgres = mockStatic(Postgres.class)) {
            ...
            assertNotNull(result, "Fetch should return a user object");
    ...
    void fetch_WithStatementCreationException_ShouldReturnNull() throws Exception {
        // Test that fetch returns null when statement creation fails
        try (MockedStatic<Postgres> mockedPostgres = mockStatic(Postgres.class)) {
            mockedPostgres.when(Postgres::connection).thenReturn(mockConnection);
    ...
    void fetch_WithQueryExecutionException_ShouldReturnNull() throws Exception {
        // Test that fetch returns null when query execution fails
        try (MockedStatic<Postgres> mockedPostgres = mockStatic(Postgres.class)) {
            mockedPostgres.when(Postgres::connection).thenReturn(mockConnection);
            when(mockConnection.createStatement()).thenReturn(mockStatement);
    ...
    void fetch_WithSpecialCharactersInUsername_ShouldExecuteQuery() throws Exception {
        ...
        try (MockedStatic<Postgres> mockedPostgres = mockStatic(Postgres.class)) {
            ...
            assertNotNull(result, "Fetch should handle special characters in username");
            assertEquals(username, result.username, "Username with special characters should be preserved");
        }
    }

    @Test
    void fetch_WithNullUsername_ShouldExecuteQueryWithNull() throws Exception {
        // Test fetch with null username
        try (MockedStatic<Postgres> mockedPostgres = mockStatic(Postgres.class)) {
            mockedPostgres.when(Postgres::connection).thenReturn(mockConnection);
            when(mockConnection.createStatement()).thenReturn(mockStatement);
            when(mockStatement.executeQuery(anyString())).thenReturn(mockResultSet);
            ...
            assertNull(result, "Fetch with null username should return null");
    ...
    void fetch_WithEmptyUsername_ShouldExecuteQueryWithEmptyString() throws Exception {
        ...
        try (MockedStatic<Postgres> mockedPostgres = mockStatic(Postgres.class)) {
            ...
            assertNull(result, "Fetch with empty username should return null");
            verify(mockStatement).executeQuery(contains("''"));
        }
    }

    @Test
    void fetch_ShouldPrintExceptionClassNameAndMessage() throws Exception {
        ...
        assertTrue(errorOutput.contains("RuntimeException"), 
                "Fetch should print exception class name");
        assertTrue(errorOutput.contains(exceptionMessage), 
                "Fetch should print exception message");
    }
}
```
